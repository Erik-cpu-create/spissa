...
fn bitplane_roundtrip_bit_exact_various_palettes() {
    ...
    for &distinct in &[1usize, 2, 3, 17, 32, 64] {
        ...
        assert!(
            ...
            "distinct={distinct}: must compress"
        );
        let dec = codec.decode(&enc.data, &dmeta()).unwrap();
        assert_eq!(
            dec, bytes,
            "distinct={distinct}: must be bit-exact lossless"
        );
    }
}
...
fn decode_neon_w5_matches_scalar_bit_for_bit() {
    ...
    for &n in &[32usize, 33, 39, 40, 47, 64, 1000, 4096, 4099] {
        ...
        assert_eq!(
            neon, scalar,
            "n={n}: NEON decode must equal scalar bit-for-bit"
        );
...
fn bitplane_neon_decode_feasibility() {
    ...
    assert_eq!(
        neon, bytes,
        "NEON decode must be lossless on the real embedding"
    );
...
fn decode16_matches_8wide_bit_for_bit() {
    ...
    for &n in &[32usize, 48, 64, 1000, 4096, 4099, 65536] {
        ...
        assert_eq!(
            sixteen, eight,
            "n={n}: 16-wide must equal 8-wide bit-for-bit"
        );
    }
}
...
fn decode16_throughput_scout() {
    let bytes =
        std::fs::read("/tmp/rllm-bf16-sample.bin").expect("run dump_bf16_embedding_sample first");
    let n = bytes.len() / 2;
    let codec = BitplaneCodec;
    let enc = codec
        .encode(
    ...
    let g8 = bench("8-wide", &|o| {
        decode_neon_w5_into(&palette, &idx_plane, &residuals, n, o)
    });
...
fn capacity_bound_stream_scout() {
    ...
    eprintln!(
        ...
        raw_ms / comp_ms,
        if comp_ms < raw_ms {
            "GO -- lossless compression WINS when model streams from SSD"
        } else {
            "NO-GO"
        }
    );
...
fn decode_neon_w5_into_matches_allocating_variant() {
    ...
    for &n in &[32usize, 33, 100, 4096, 4099] {
        ...
        assert_eq!(
            into, alloc,
            "n={n}: decode_neon_w5_into must match decode_neon_w5"
        );
    }
}
...
fn decode16_w6_matches_scalar_bit_for_bit() {
    ...
    for &n in &[34usize, 35, 47, 48, 64, 80, 96, 1000, 4096, 4099, 65536] {
        ...
        assert_eq!(
            neon, scalar,
            "n={n}: w=6 16-wide must equal scalar bit-for-bit"
        );
...
fn decode_bitplane_row_dispatch_matches_scalar() {
    ...
    for &(distinct, w) in &[(32usize, 5u8), (34usize, 6u8)] {
        for &n in &[64usize, 1000, 4099] {
            ...
            assert_eq!(
                dispatched, bytes,
                "distinct={distinct} n={n}: dispatch must be lossless"
            );
        }
    }
}
