...
//
// Run: cargo test -p rllm-runtime --release --test weight_entropy_analysis -- --ignored --nocapture
//
// Uses only the public API (LazyRllmModel) on the raw-codec model (every tensor is
...
fn weight_entropy_floor_real_weights() {
    let mut m = LazyRllmModel::open(MODEL)
