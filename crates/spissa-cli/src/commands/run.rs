...
fn run_generation(
    ...
    let mut model =
        LazySpissaModel::open(path).with_context(|| format!("failed to open {}", file))?;
    ...
    let tokenizer = model
        ...
        .tokenizer
        .clone()
        .map(|metadata| SpissaTokenizer::from_metadata(&metadata))
        .transpose()?;
...
fn run_planned(
    ...
    let model = LazySpissaModel::open(path).with_context(|| format!("failed to open {}", file))?;
