...
fn format_rolling_note(stats: spissa_runtime::RamaRollingStats) -> String {
    if stats.is_empty() {
        String::new()
    } else {
        format!(
...
fn format_aip_note(stats: spissa_runtime::RamaExperimentalSpeedStats) -> String {
    if stats.is_empty() {
        String::new()
    } else {
        ...
        let lm_head_rescore_note = if stats.lm_head_rescore_checks > 0 {
            format!(
                " aip_lm_head_rescore={}/{} aip_lm_head_rescore_gap_skips={} aip_lm_head_rescore_max_gap_milli={}",
        ...
        let attention_locality_note = if stats.attention_locality_uses > 0 {
            format!(
                " aip_attention_locality_added={}/{} aip_attention_locality_max_selected={}",
        ...
        let lm_head_phrase_novelty_note = if stats.lm_head_phrase_novelty_checks > 0 {
            ...
            let retention_note = if stats.lm_head_phrase_novelty_retentions > 0 {
                format!(
                    " aip_lm_head_phrase_novelty_retentions={}",
                    stats.lm_head_phrase_novelty_retentions
                )
            } else {
                String::new()
            };
...
fn format_repetition_note(stats: spissa_runtime::RamaRepetitionStats) -> String {
...
fn replay_command(
    ...
    format!(
        "cargo run --release -p spissa-cli -- chat-session-token {}{} --max-new-tokens {max_new_tokens} --ctx {ctx} --out {}",
        shell_quote(file),
...
mod tests {
    ...
    use spissa_runtime::{RamaSessionPhaseTimings, RamaTransformerPhaseTimings};
    ...
    fn format_rolling_note_reports_nonzero_stats() {
        let note = format_rolling_note(spissa_runtime::RamaRollingStats {
            submitted_tasks: 4,
            worker_wakeups: 4,
    ...
    fn format_aip_note_reports_nonzero_stats() {
        let note = format_aip_note(spissa_runtime::RamaExperimentalSpeedStats {
        ...
        assert!(note.contains("aip_lm_head_rescore=6/9"));
        assert!(note.contains("aip_lm_head_rescore_gap_skips=3"));
        assert!(note.contains("aip_lm_head_rescore_max_gap_milli=450"));
        ...
        assert!(note.contains("aip_attention_locality_added=18/6"));
        assert!(note.contains("aip_attention_locality_max_selected=8"));
        ...
        assert!(note.contains("aip_lm_head_exact=2/5"));
    ...
    fn format_repetition_note_reports_nonzero_stats() {
        let note = format_repetition_note(spissa_runtime::RamaRepetitionStats {
            generated_tokens: 10,
