...
fn write_report(
    ...
    max_new_tokens: usize,
    ctx: usize,
    turns: &[(usize, String, spissa_runtime::RamaSessionTurnResult)],
) -> Result<()> {
    ...
    let replay_command = format!(
        "cargo run -p spissa-cli -- chat-session {}{} --max-new-tokens {max_new_tokens} --ctx {ctx} --out {}",
        shell_quote(file),
...
mod tests {
    ...
    fn tokenizer(tokens: &[&str]) -> SpissaTokenizer {
        SpissaTokenizer::from_metadata(&TokenizerMetadata {
