...
use spissa_runtime::{
    ...
    LazySpissaModel, MemoryBudget, RamaIntegrityMode, SpissaTokenizer, StreamingSamplingConfig,
};
...
fn main() -> Result<()> {
    ...
    // and kernels read these via SPISSA_MLOCK / SPISSA_Q8_ACTIVATION (the env-gated
    ...
    // per session). SPISSA_INTEGRITY={unchecked,verifyonce,strict} overrides it —
    // a diagnostic/operational knob to measure or skip the verification cost.
    let integrity_mode = match std::env::var("SPISSA_INTEGRITY").ok().as_deref() {
        ...
        Some(other) => anyhow::bail!(
            "invalid SPISSA_INTEGRITY={other:?} (expected unchecked|verifyonce|strict)"
        ),
    };
    ...
    let tokenizer = model
        ...
        .tokenizer
        .clone()
        .map(|meta| SpissaTokenizer::from_metadata(&meta))
        .transpose()?;
