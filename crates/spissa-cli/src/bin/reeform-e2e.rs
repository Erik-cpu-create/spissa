...
fn main() -> anyhow::Result<()> {
    ...
    for name in &bnames {
        ...
        let dmeta = DecodeMeta {
            ...
            uncompressed_size: d_bytes.len() as u64,
        };
        let dec = RansCodec.decode(&cd, &dmeta)?;
        let dec16: Vec<u16> = (0..dec.len() / 2)
            .map(|i| u16::from_le_bytes([dec[2 * i], dec[2 * i + 1]]))
            .collect();
        for i in 0..n {
    ...
    println!(
        "  rANS(full fine-tune)  baseline = {bft:.4} bit/weight   ({} bytes)",
        comp_ft
    );
    println!(
        "  rANS(delta)  OURS              = {bdelta:.4} bit/weight   ({} bytes)",
        comp_delta
    );
    println!(
        "  reconstruction ft = base + Δ   : {}",
        if all_exact {
            "✅ BIT-EXACT"
        } else {
            "❌ MISMATCH (bug)"
    ...
    if win > 0.05 && all_exact {
        ...
        println!(
            "     {} MB → {} MB per fine-tune (base stored once)",
            comp_ft / 1_048_576,
            comp_delta / 1_048_576
        );
    } else {
