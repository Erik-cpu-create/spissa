...
use spissa_container::{ChunkMeta, GlobalMetadata, SpissaReader, TensorMeta};
...
pub struct LazySpissaModel {
    ...
    /// bf16-resident speed instead of per-token re-decode. Opt-in (SPISSA_DECODE_RESIDENT=1);
...
impl LazySpissaModel {
    ...
    pub fn open(path: impl AsRef<Path>) -> Result<Self> {
        ...
        let reader = SpissaReader::open(&path_buf)?;
...
mod tests {
    ...
    /// Run: `cargo test -p spissa-runtime --release dump_bf16_embedding_sample -- --ignored --nocapture`
    #[test]
    #[ignore]
    fn dump_bf16_embedding_sample() {
        ...
        let mut m = LazySpissaModel::open(path).unwrap();
    ...
    /// `cargo test -p spissa-runtime --release q8_vs_bf16_quantization_error -- --ignored --nocapture`
    #[test]
    #[ignore]
    fn q8_vs_bf16_quantization_error() {
        ...
        let mut q8 = LazySpissaModel::open(q8_path).expect("open q8");
        let mut raw = LazySpissaModel::open(raw_path).expect("open raw bf16");
    ...
    fn open_reads_metadata_without_decoding_payloads() {
        ...
        let model = LazySpissaModel::open(&path).unwrap();
    ...
    fn prewarm_chunk_integrity_verifies_chunks_and_enables_the_bridge() {
        ...
        for mode in [RamaIntegrityMode::Unchecked, RamaIntegrityMode::Strict] {
            let mut other = LazySpissaModel::open(&path).unwrap();
