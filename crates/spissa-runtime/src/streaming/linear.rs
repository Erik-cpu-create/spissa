...
fn quantized_elements_for_bytes(dtype: spissa_container::DType, bytes_len: usize) -> Result<usize> {
...
fn dequantize_quantized_chunk(
    ...
    match dtype {
        spissa_container::DType::Q4_0 => crate::dequantize::dequantize_q4_0(bytes, &mut weights),
        spissa_container::DType::Q8_0 => crate::dequantize::dequantize_q8_0(bytes, &mut weights),
        _ => {
            return Err(RuntimeError::InvalidTensorData(format!(
...
pub fn streaming_tile_linear_from_model(
    ...
    // SPISSA_Q8_ACTIVATION=0). View the whole tensor as one contiguous mmap slice and
    ...
    // R138 prefill fast-path: raw-codec Q8_0 + batch>=2 (gated on SPISSA_Q8_ACTIVATION).
    ...
    for chunk in chunks {
        ...
        if chunk.codec_id == "rtc-rle-v1" && tensor.dtype == spissa_container::DType::U8 {
        ...
        } else if chunk.codec_id == "rtc-raw-v1" && tensor.dtype == spissa_container::DType::Fp16 {
            model.with_raw_chunk(chunk.chunk_id, budget, |compressed_bytes, _budget| {
        ...
        } else if tensor.dtype == spissa_container::DType::Bf16 {
            ...
            // bf16 via `with_decoded_chunk` (cached when SPISSA_DECODE_RESIDENT) and feed the
