...
remaining: generic shape/budget-aware prefill policy is implemented in Phase 7.12A; next optimize measured Pythia-160M MLP/QKV projection bottlenecks; keep further MLP/QKV/decode/lm-head work evidence-driven; consider low-RAM parallel row-span accumulation only if short-prompt decode/lm-head becomes the priority; evaluate true intra-chunk compressed range decode only when measured trade-offs justify it; after Pythia-family scale tuning, start LLaMA-family architecture expansion with a new adapter rather than model-size-specific hacks
```
